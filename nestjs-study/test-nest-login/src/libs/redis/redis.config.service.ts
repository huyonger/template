...
export class RedisConfigService{
  ...
  private async initRedis(connectType ?: string) {
    if (connectType && connectType === 'cluster') {
      const cluster = new Redis.Cluster(this.config.get('redisClusterConfig'));
   ...
   * @returns 
   */
  async setRedis(key, value:any, time?: number): Promise<Boolean> {
    let isOk = true
    // 存储
   ...
   */
  async setRedisTime(key,time:number){
   ...
   */
    async delRedis(key){
      return this.setRedisTime(key, 1)
    }
